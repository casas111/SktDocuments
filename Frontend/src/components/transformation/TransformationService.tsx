...
import EnhancedTransformationNode from './EnhancedTransformationNode';
import CustomInstructionEditor from '../common/CustomInstructionEditor';
import DocumentSelector from '../documents/DocumentSelector';
import TransformationsOutput from '../documents/TransformationsOutput';
...
// Styled components
const TransformationServiceContainer = styled(Box)`
  padding: 24px;
  max-width: 1200px;
  margin: 0 auto;
`;
...
const TransformationService: React.FC = () => {
  // Document state
  ...
  const handleProcessTransformation = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/transformation/trigger`, {
        ...
        sourceDocIds: [sourceDoc1!.id, sourceDoc2!.id]
      });
      
      if (response.data.success) {
        setNotification({
          open: true,
          message: 'Transformation processed successfully',
          severity: 'success'
        });
        ...
        if (response.data.data && response.data.data.transformedDocument) {
          setOutputDoc(response.data.data.transformedDocument);
        }
      } else {
        setNotification({
          open: true,
          message: response.data.error || 'Error processing transformation',
          severity: 'error'
        });
      }
    } catch (error: unknown) {
      ...
      setNotification({
        open: true,
        message: error instanceof Error && error.message ? error.message : 'Error processing transformation',
        severity: 'error'
      });
    } finally {
      setLoading(false);
  ...
  return (
    <TransformationServiceContainer>
      ...
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 4 }}>
        <Box sx={{ flex: 1, minWidth: { xs: '100%', md: '60%' } }}>
          ...
          <Box sx={{ mt: 3, display: 'flex', gap: 2 }}>
            <Button 
              ...
              sx={{ flex: 1 }}
            >
              {loading ? 'Processing...' : 'Process Transformation'}
            </Button>
        ...
        <Box sx={{ flex: 1, minWidth: { xs: '100%', md: '40%' } }}>
          ...
          <Typography variant="body2" color="text.secondary" paragraph>
            This is how the transformation node will appear in the workflow canvas.
          </Typography>
